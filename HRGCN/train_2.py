...
class Train2(object):
        ...
        if self.model_version == 11:
            ...
            if self.dataset_id == 0:
                self.dataset = HetGCNEventGraphDataset(
                    node_feature_csv=f"{self.data_root_dir}/train_node_feature_norm.csv",
                    edge_index_csv=f"{self.data_root_dir}/train_edge_index.csv",
    ...
    def train(self):
        ...
        benign_gid_list = [i for i in range(self.num_train_benign)]
    ...
    def eval_model(self, eval_list=None):
        """
        Eval Model
        TODO: Fill eval logic for reading valid data
        """
        if self.num_eval:
